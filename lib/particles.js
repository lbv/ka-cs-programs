 ...
 *     If the optional timestep is given, then every it will
 *     be used as the fixed time interval between frames,
 *     otherwise it will obtain it from the environment, and
 *     it can fluctuate.
 *
 *   newParticle(x, y, invMass)
 *     Creates a particle with position (x, y), and a mass
 *     of 1/invMass
 *
 *   pushForce(force)
 *     Adds a new global force (which must be a PVector)
 *     that applies to all particles
 *
 *   popForce()
 *     Removes the force on the top of the stack
 *
 *   addConstraint(p1, p2)
 *     Adds a contraint between particles (p1, p2), so the
 *     distance between them tries to be maintained
 *
 *   advance()
 *     Moves the simulation. This should be done at each
 *     frame (e.g., calling it from the 'draw' function).
 */
var ParticleSystem = function(ts) {
	this.ps = {};        // particles
	...
	this.tStep = ts === undefined ? null : ts;
};
...
ParticleSystem.prototype.advance = function() {
	var now = this.tStep === null ?
		millis() : this.t1 + this.tStep;
