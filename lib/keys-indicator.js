...
KeysIndicator.colorBG = 0xff204a87;
KeysIndicator.colorFG = 0xffffffff;

KeysIndicator.prototype.drawKey = function(key) {
	stroke(KeysIndicator.colorBG);
	strokeWeight(1.5);
	if (key.on) { fill(KeysIndicator.colorBG & 0xccffffff); }
	else        { noFill(); }
	rect(0, 0, KeysIndicator.size, KeysIndicator.size, KeysIndicator.radius);
	if (key.on) { fill(KeysIndicator.colorFG); }
	else        { fill(KeysIndicator.colorBG); }
